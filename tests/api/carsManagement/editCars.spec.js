...
test.describe("PUT", () => {
  ...
  test("should edit a car by returned id", async () => {
    const brandId = VALID_BRANDS_RESPONSE_BODY.data[0].id;
    const modelId = VALID_BRAND_MODELS[brandId].data[0].id;
    ...
    const newBrandId = VALID_BRANDS_RESPONSE_BODY.data[1].id;
    const newModelId = VALID_BRAND_MODELS[newBrandId].data[0].id;
    const editRequestBody = {
      carBrandId: newBrandId,
    ...
    expect(
      ...
      "Car should be edited with data from request",
    ).toMatchObject({
      carBrandId: newBrandId,
      carModelId: newModelId,
      ...editRequestBody,
